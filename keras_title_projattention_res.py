...
def find_ranking_quick(projection1, projection2, dlmodel):
    ...
    # Compile the model
    if mode == "reg":
        model_1.compile(optimizer=optimizertype, loss=loss_function)
    else:
        model_1.compile(optimizer=optimizertype, loss='binary_crossentropy', metrics=['accuracy'])
   ...
   # if bi_lstm_mode == "sum" or bi_lstm_mode == "mul":
    ...
    merged_withatt_res = Dense(dim_lstm)(merged_withatt)
    
    merged_vector_res = keras.layers.Add()([merged_vector, merged_withatt_res])

    # (Dense 1) * 3
    #x1 = Dense(dim_1, activation=p_activation[0])(merged_vector)
    x1 = Dense(dim_1, activation=p_activation[0])(merged_vector_res)
    ...
    model_2.layers[13].set_weights(dlmodel.layers[15].get_weights())

    # Compile the model
    if mode == "reg":
        model_2.compile(optimizer=optimizertype, loss=loss_function)
    else:
        model_2.compile(optimizer=optimizertype, loss='sparse_categorical_crossentropy', metrics=['accuracy'])
...
if __name__ == "__main__":
   ...
   #print(K.int_shape(at1))
    ...
    model_lstm2.compile(optimizer=optimizertype, loss=loss_function)
    # model.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])
    ...
    for ep in range(epoch):
        ...
        for sp in range(div):
            ...
            if mode == "reg":
                y = np.concatenate((np.ones(len(features_en_1[rag_left:rag_right])),
                                    np.zeros(len(features_en_0[rag_left:rag_right])) + bias_y), axis=0)
            # y = np.concatenate((np.ones(len(features_en_1)), np.zeros(len(features_en_0))), axis = 0)
            else:
                y = np.concatenate(
                    (np.ones(len(features_en_1[rag_left:rag_right])), np.zeros(len(features_en_0[rag_left:rag_right]))),
                    axis=0)
            ...
            if mode == "reg":
                y_test_new = np.concatenate((np.ones(len(features_en_new)), np.zeros(len(features_en_0_new)) + bias_y),
                                            axis=0)
            # y_test = np.concatenate((np.ones(len(X1_test_1)), np.zeros(len(X1_test_0))), axis = 0)
            else:
