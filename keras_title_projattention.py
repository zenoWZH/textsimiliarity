...
from keras.layers import Input, Embedding, LSTM, Dense, Dropout, Bidirectional, Softmax, Concatenate
...
parser.add_argument("--path", help="load model")
...
def find_ranking_batch(projection1, projection2, dlmodel, batch=100):
    sim_results = []
    rank_results = []
...
def find_ranking_quick(projection1, projection2, dlmodel):
    ...
    rank_results = []
    start_time = time.time()
    print("Find ranking of ", len(projection2), " sentence pairs")
    ...
    # LSTM layer
    if rnn_type == "lstm":
        lstm_out_1 = LSTM(dim_lstm, go_backwards=True, return_sequences = True)(input_1)
        lstm_out_2 = LSTM(dim_lstm, go_backwards=True, return_sequences = True)(input_2)
    elif rnn_type == "bi-lstm":
        lstm_out_1 = Bidirectional(LSTM(dim_lstm, go_backwards=True, return_sequences = True), merge_mode=bi_lstm_mode)(input_1)
        lstm_out_2 = Bidirectional(LSTM(dim_lstm, go_backwards=True, return_sequences = True), merge_mode=bi_lstm_mode)(input_2)
   ...
   # if bi_lstm_mode == "sum" or bi_lstm_mode == "mul":
    #    co = 1
    #print(K.int_shape(lstm_out_1))
    input_1 = Input(shape=(maxlen,dim_lstm), dtype='float32', name='lstm1')
    input_2 = Input(shape=(maxlen,dim_lstm), dtype='float32', name='lsmt2')

    score_1 = keras.layers.dot([input_1,  input_2], axes = [2,2])
    score_2 = keras.layers.dot([ input_2, input_1], axes = [2,2])
    #print(K.int_shape(score_1))

    at1 = Softmax(axis = 2)(score_1)
    at2 = Softmax(axis = 2)(score_2)
    #print(K.int_shape(at1))
    #ct1 = K.batch_dot(score_1,lstm_out_2, axes=[2, 1])
    #ct2 = K.batch_dot(score_2,lstm_out_1, axes=[2, 1])
    ct1 = keras.layers.dot([score_1,  input_2], axes=[2, 1])
    ct2 = keras.layers.dot([score_2, input_1], axes=[2, 1])
    #print(K.int_shape(ct1))

    # must use as keras style
    #origin_1 = lstm_out_1[:,-1,:]
    #origin_2 = lstm_out_2[:,-1,:]
    #attout_1 = ct1[:,-1,:]
    #attout_2 = ct2[:,-1,:]

    getlastoutput = Lambda(lambda x: x[:, -1, :])
    origin_1 = getlastoutput(input_1)
    origin_2 = getlastoutput( input_2)
    attout_1 = getlastoutput(ct1)
    attout_2 = getlastoutput(ct2)

    # Merge attention with output of LSTM
    # Seperately
    # merged_1 = keras.layers.concatenate([origin_1, attout_1], axis=-1)
    # merged_2 = keras.layers.concatenate([origin_2, attout_2], axis=-1)

    # withatt_out_1 = Dense(dim_lstm, activation = 'relu')(merged_1)
    # withatt_out_2 = Dense(dim_lstm, activation = 'relu')(merged_2)

    # Seperately Merge layer
    # merged_vector = keras.layers.concatenate([withatt_out_1, withatt_out_2], axis=-1)

    # Together
    merged_withatt = keras.layers.concatenate([origin_1, attout_1, origin_2, attout_2], axis=-1)

    merged_vector = Dense(dim_lstm, activation=att_activation[0])(merged_withatt)
    ...
    # Set the weights
    model_2.layers[8].set_weights(dlmodel.layers[10].get_weights())
    model_2.layers[9].set_weights(dlmodel.layers[11].get_weights())
    model_2.layers[11].set_weights(dlmodel.layers[13].get_weights())
    ...
    # release memory
    del input_1, input_2, model_1, model_2, lstm_out_1, lstm_out_2, main_output, projection1, projection2, v1, v2, v3, origin_1, origin_2, attout_1, attout_2, merged_withatt, merged_vector, x1
    gc.collect()
...
if __name__ == "__main__":
    ...
    # ------------------- Model ------------------ #
    # Input layer
    input_1 = Input(shape=(maxlen, dim_lstm), dtype='float32', name='main_input_1')
    input_2 = Input(shape=(maxlen, dim_lstm), dtype='float32', name='main_input_2')
    ...
    #hs1 = Permute((2,1))(lstm_out_1)
    #hs2 = Permute((2,1))(lstm_out_2)
    
    #K1 = K.int_shape(lstm_out_1)
    #K2 = K.int_shape(lstm_out_2)
    #print(K1)
    #print(K2)

    #score_1 = K.batch_dot(lstm_out_1,lstm_out_2, axes= [2,2] )
    #score_2 = K.batch_dot(lstm_out_2,lstm_out_1, axes= [2,2] )
    score_1 = keras.layers.dot([lstm_out_1, lstm_out_2], axes = [2,2])
    score_2 = keras.layers.dot([lstm_out_2, lstm_out_1], axes = [2,2])
    #print(K.int_shape(score_1))
   ...
   #print(K.int_shape(at1))
    #ct1 = K.batch_dot(score_1,lstm_out_2, axes=[2, 1])
    #ct2 = K.batch_dot(score_2,lstm_out_1, axes=[2, 1])
    ct1 = keras.layers.dot([score_1, lstm_out_2], axes=[2, 1])
    ct2 = keras.layers.dot([score_2, lstm_out_1], axes=[2, 1])
    #print(K.int_shape(ct1))
    ...
    getlastoutput = Lambda(lambda x: x[:, -1, :])
    origin_1 = getlastoutput(lstm_out_1)
    origin_2 = getlastoutput(lstm_out_2)
    attout_1 = getlastoutput(ct1)
    attout_2 = getlastoutput(ct2)
    ...
    for ep in range(epoch):
        ...
        for sp in range(div):
            ...
            # y = np.concatenate((np.ones(len(features_en_1)), np.zeros(len(features_en_0))), axis = 0)
                ...
                y1 = np.concatenate(((np.zeros((len(features_en_1[rag_left:rag_right]),1)), np.ones((len(features_en_1[rag_left:rag_right]),1)))), axis=1)
                y0 = np.concatenate(((np.ones((len(features_en_0[rag_left:rag_right]),1)), np.zeros((len(features_en_0[rag_left:rag_right]),1)))), axis=1)
            ...
            # y_test = np.concatenate((np.ones(len(X1_test_1)), np.zeros(len(X1_test_0))), axis = 0)
                ...
                #y_test = np.concatenate((np.ones(len(X1_test_1)), np.zeros(len(X1_test_0))), axis=0)
                y_test = np.concatenate((y_test,Y_o),axis=0)
            ...
            # y_test = np.concatenate((np.ones(len(X1_test_1)), np.zeros(len(X1_test_0))), axis = 0)
                ...
                y1 = np.concatenate(((np.zeros((len(features_en_new),1)), np.ones((len(features_en_new),1)))), axis=1)
                y0 = np.concatenate(((np.ones((len(features_en_0_new),1)), np.zeros((len(features_en_0_new),1)))), axis=1)
                y_test_new = np.concatenate(((y1,y0)), axis=0)
        ...
        if mode == "reg":
            ...
            for i in [1, 5, 10]:
                ...
                print("[T] TOP", i, top_test)
                    ...
                    # release memory
            del  rank_results_test2, sim_results_test2
            gc.collect()
        else:
            ...
            tops[1].append(metrics.precision_score(y_test, y_pred_test1>0.5,average='macro'))
            tops[5].append(metrics.recall_score(y_test, y_pred_test1>0.5,average='macro'))
            tops[10].append(metrics.f1_score(y_test, y_pred_test1>0.5,average='macro'))
            tops_test[1].append(metrics.precision_score(y_test_new, y_pred_test2>0.5,average='macro'))
            tops_test[5].append(metrics.recall_score(y_test_new, y_pred_test2>0.5,average='macro'))
            tops_test[10].append(metrics.f1_score(y_test_new, y_pred_test2>0.5,average='macro'))
            
            # release memory
            del y_pred_train1, y_pred_test1, y_pred_test2
            gc.collect()
